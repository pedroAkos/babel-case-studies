...
for run in $(# ------------------------------------------- RUN
  ...
  for n_servers in "${n_servers_list[@]}"; do # --------------------------- N_SERVERS
    ...
    for snode in "${server_nodes[@]}"; do
      nodeip=$(getent hosts "${snode}" | awk '{ print $1 }')
      ips_with_port=${ips_with_port}${nodeip}":50400,"
    done
    client_connections=${server_nodes[0]}
    ips_with_port=${ips_with_port::-1}
    echo " -- -- - Client connections: ${client_connections}"
    echo " -- -- - Server ips: ${ips_with_port}"
    ...
    for reads_per in "${reads_list[@]}"; do # ---------------------------  READS_PER
      ...
      for payload in "${payloads_list[@]}"; do # ------------------------- PAYLOADS
        ...
        exp_path_client="../logs/wpaxos/${exp_name}/client/${n_servers}/${reads_per}/${payload}/${run}"
        exp_path_server="../logs/wpaxos/${exp_name}/server/${n_servers}/${reads_per}/${payload}/${run}"
        ...
        for n_threads in "${threads_list[@]}"; do # -------------------- N_THREADS
          ...
          for server_node in "${server_nodes[@]}"; do
            myipport=$(getent hosts "${server_node}" | awk '{ print $1 }')":50400"
            echo "myipport ${myipport}"
            oarsh "$server_node" "rm -rf /tmp/wpaxos; cd babel/wpaxos && java -Xmx${xmx} -Xms${xms} \
											-Dlog4j.configurationFile=log4j2.xml \
											-Djava.net.preferIPv4Stack=true \
											-DlogFilename=${exp_path_server}/${n_threads}_${server_node} \
											-cp wpaxos.jar \
											novasys.app.HashMapApp /tmp/wpaxos ${myipport} ${ips_with_port} 50500" 2>&1 | sed "s/^/[s-$server_node] /" &
            sleep 1
            i=$((i+1))
            server_p_ids+=($!)
          done
