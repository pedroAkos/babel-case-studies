...
for run in $(# ------------------------------------------- RUN
  ...
  for n_servers in "${n_servers_list[@]}"; do # --------------------------- N_SERVERS
    ...
    for snode in "${server_nodes[@]}"; do
      ...
      ips_without_port=${ips_without_port}${nodeip}","
    done
    client_connections=${server_nodes[0]}
    ips_without_port=${ips_without_port::-1}
    echo " -- -- - Client connections: ${client_connections}"
    echo " -- -- - Server ips: ${ips_without_port}"
    ...
    for reads_per in "${reads_list[@]}"; do # ---------------------------  READS_PER
      ...
      for payload in "${payloads_list[@]}"; do # ------------------------- PAYLOADS
        ...
        exp_path_client="../logs/mypaxos/${exp_name}/client/${n_servers}/${reads_per}/${payload}/${run}"
        exp_path_server="../logs/mypaxos/${exp_name}/server/${n_servers}/${reads_per}/${payload}/${run}"
        ...
        for n_threads in "${threads_list[@]}"; do # -------------------- N_THREADS
          ...
          for server_node in "${server_nodes[@]}"; do
            oarsh "$server_node" "cd babel/mypaxos && java -Xmx${xmx} -Xms${xms} \
											-Dlog4j.configurationFile=log4j2.xml \
											-Djava.net.preferIPv4Stack=true \
											-DlogFilename=${exp_path_server}/${n_threads}_${server_node} \
											-cp mypaxos.jar novasys.app.HashMapApp ./conf/conf${i}.json ${ips_without_port}" 2>&1 | sed "s/^/[s-$server_node] /" &
            sleep 1
            i=$((i+1))
            server_p_ids+=($!)
          done
